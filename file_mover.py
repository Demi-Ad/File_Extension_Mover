...
def files_mover(target_dir, set_dir , filter_files ):
    ...
    result_time = datetime.today().strftime("%Y%m%d%H%M%S") # 결과 파일명으로 지정할 현재 날짜 , 시간을 구함
        ...
        for move_file in filter_files:
            if move_dir.split('\\')[-1] == move_file.split('.')[-1]:
                if os.path.isfile(move_dir +'\\'+ move_file): # 이동시킬 경로에 이름이 같은 파일이 있으면
                    shutil.move(target_dir+'\\'+move_file , move_dir +'\\'+ '(' + str(file_counter) + ')' + move_file) # 파일명 앞에 (숫자) 붙여서 덮어쓰기 방지
                    print(target_dir+'\\'+move_file + ' - - - ----> ' + move_dir +'\\'  + '(' + str(file_counter) + ')' + move_file)
                    file_counter += 1 # 숫자 증가
                    file.writelines(target_dir+'\\'+move_file + '- - - - ->' + move_dir + '(' + str(file_counter) + ')' + move_file+'\n')
                else: 
                    shutil.move(target_dir+'\\'+move_file , move_dir +'\\'+ move_file ) # 파일이동
                    print(target_dir+'\\'+move_file + ' - - - ----> ' + move_dir +'\\'+ move_file)
                    file.writelines(target_dir+'\\'+move_file + '- - - - ->' + move_dir +'\\'+ move_file +'\n')
    
    file.close()


def main(src , args): # 정리할 경로 , 정리할 확장자들의 리스트
    
    # 입력값이 '/' 이라면 현재 경로 아니면 입력한 경로
    if src == '/':
        target_dir = os.getcwd() # 현재 경로를 입력
        is_me = inspect.getfile(inspect.currentframe()).split('\\')[-1] # 파일 자기 자신 경로
        files = os.listdir(target_dir) # 입력한 경로에 파일목록을 추출
        filter_files = [x for x in files if files_filter(x)] # 리스트 표현식을 이용해서 확장자가 없는 파일 , 폴더 필터
    ...
    for i in result:
        extensions.append(i.split('.')[1]) # 확장자가 있는 파일들을 '.'으로 스플릿 후 2번째 확장자만 추출
